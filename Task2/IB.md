...
|8| Используются ли системы аутентификации пользователей для доступа к системам?                  |Да|AD, keycloak| 
|9| Созданы ли замкнутые контуры для подсистем, работающих с чувствительными данными?      |Нет|Отделен бухгалтерский сегмент, но ПнД используемые при заключении сделок не отделены.| 
|10| Ограничен ли доступ к системам из внешних источников?          |Нет|Нужна проверка политик МЭ, до проверки считаем что ограничений нет| 
|11| Ведётся ли единый реестр учётных записей?             |Нет|Есть отдельные два AD и keycloak| 
...
|4| Проводится ли регулярное резервное копирование данных? Хранятся ли резервные копии в защищённых хранилищах?    |Нет|Копии делаются, но нет точной информации где они хранятся| 
...
|1| Используется ли межсетевой экран для ограничения входящего и исходящего трафика?                              |Нет|Нужна проверка политик МЭ, до проверки считаем что фаервол пропускает любой трафик| 
|2| Реализована ли сегментация сети на логические зоны (DMZ, корпоративная сеть, гостевая сеть) для ограничения доступа к критическим ресурсам? |Нет|Нужна проверка настроек интерфейсов МЭ, большая вероятность, что inside интерфейс смотрит в зону КСПД, а outside в зону public_internet| 
|3| Используются ли IDS/IPS-системы для обнаружения и предотвращения сетевых атак?              |Нет|Нужна проверка политик МЭ, возможно наличие данного функционала на outside интерфейсе| 
...
|12| Используются ли средства для контроля периметра доступа?                 |Да|МЭ установлен, нужно проверять как настроен| 
...
|4| Настроены ли политики автоматического резервного копирования в облачной среде? Контролируется ли доступ к резервным копиям?   |Нет|Копии делаются, доступ к копиям не ограничен| 
| **V.2. Управление оборудованием и физической безопасностью**                                                             |       |              || 
|1| Используются ли системы контроля доступа — биометрия, СКУД — в серверных помещениях?                     |Нет|Заявлен собственный ЦОД, без уточнения его характеристик, большая вероятность реального наличия СКУД   | 
